...
for run in runs:
    ...
    run_segments, _, run_classes, run_n_classes = dataset.load(
...
if config.roc:
    ...
    # Flatten pair_ids to ensure it contains only integers
    flat_pair_ids = [item for sublist in pair_ids for item in sublist]

    # Now find unique elements, which will be integers
    roc_ids = np.unique(flat_pair_ids)
...
# train_fold = np.ones(classes.shape, dtype=int)
...
#     # Randomly choose the duplicate class for training set
#     dup_keep = np.random.choice(unique_dup_classes)
#     #print(f"Class {c}, Keeping duplicate class {unique_dup_classes[dup_keep]} in training set.")

#     # randomly distribute the others
#     for i, dup_class in enumerate(unique_dup_classes):
#         if i != dup_keep:
#             random_value = np.random.random()
#             if random_value < config.test_size:
#             #if np.random.random() < config.test_size:
#                 #print(f"  Random value: {random_value}, Adding duplicate class {dup_class} to test set.")
...
#print(f"Number of samples in test set: {len(test_ids)}")
...
# Convert classes to a 1D array if it's not already
classes_1d = np.argmax(classes, axis=1) if classes.ndim > 1 else classes

for fold, (train_ids, test_ids) in enumerate(skf.split(segments, classes_1d)):
    print(f"Training on fold {fold+1}/{n_splits}")

    # Initialize your preprocessor and generators for this fold
    preprocessor.init_segments(segments, classes, train_ids=train_ids)
    gen_train = Generator(preprocessor, train_ids, n_classes, train=True, batch_size=config.batch_size, shuffle=True)
    gen_test = Generator(preprocessor, test_ids, n_classes, train=False, batch_size=config.batch_size, shuffle=True)
    ...
    # Training and Testing loop for this fold
    # ...

    # At the end of each fold, record the performance metrics
    # ...
        ...
        for epoch in tqdm(range(0, config.n_epochs)):
            ...
            for step, train in enumerate(batches):
                ...
                if train:
                    ...
                    #print("batch_segments shape train:", batch_segments.shape)
                    ...
                    #print("batch_segments shape test: ", batch_segments.shape)
